...
public class DBConfig extends SQLiteOpenHelper {
    ...
    public Cursor searchByTitle(String title) {
        SQLiteDatabase db = getReadableDatabase();
        return db.rawQuery("SELECT * FROM " + TABLE_NAME + " WHERE " + COLUMN_JUDUL + " LIKE ?", new String[]{"%" + title + "%"});
    }
    ...
    public void updateRecord(int id, String judul, String deskripsi) {
        ...
        values.put(COLUMN_JUDUL, judul);
        values.put(COLUMN_DESKRIPSI, deskripsi);
        values.put(COLUMN_UPDATED_AT, getCurrentDateTime()); // Perbarui kolom updated_at
