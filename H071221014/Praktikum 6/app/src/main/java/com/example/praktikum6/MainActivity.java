...
public class MainActivity extends AppCompatActivity {
    ...
    private void loadMoreData() {
        currentPage++;
        progressBar.setVisibility(View.VISIBLE); // Show progress bar before executing background task
        executorService.execute(new Runnable() {
            @Override
            public void run() {
                try {
                    Response<UserResponse> response = apiService.getUsers(currentPage, 6).execute();
                    if (response.isSuccessful()) {
                        final List<User> newUsers = response.body().getData();
                        if (!newUsers.isEmpty()) {
                    ...
                    recyclerView.post(new Runnable() {
                        @Override
                        public void run() {
                            progressBar.setVisibility(View.GONE); // Hide progress bar after executing background task
                        }
                    });
                }
            }
